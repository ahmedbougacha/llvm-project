...
#define DEBUG_TYPE "llvm-stable-hash"
...
uint64_t llvm::getStableStringHash(StringRef string) {
...
uint64_t llvm::getPointerAuthStringDiscriminator(StringRef string) {
  ...
  LLVM_DEBUG(
    dbgs() << "Ptrauth string disc: " << utostr(discriminator)
           << " (0x" << utohexstr(discriminator) << ")"
           << " of: " << string << "\n");
