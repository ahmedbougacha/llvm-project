...
; CHECK-NEXT: Lloh{{.*}}:
; CHECK-NEXT:         adrp [[STR_TMP:x[0-9]+]], l_hello_str@PAGE
; CHECK-NEXT: Lloh{{.*}}:
; CHECK-NEXT:         add [[STR:x[0-9]+]], [[STR_TMP]], l_hello_str@PAGEOFF
; CHECK-NEXT:         str [[STR]], [x0]
; CHECK-NEXT: [[PRECALL:L.*]]:
; CHECK-NEXT: Lloh{{.*}}:
; CHECK-NEXT:         adrp x[[ZTI_TMP:[0-9]+]], __ZTIPKc@GOTPAGE
; CHECK-NEXT: Lloh{{.*}}:
; CHECK-NEXT:         ldr x1, [x[[ZTI_TMP]], __ZTIPKc@GOTPAGEOFF]
; CHECK-NEXT:         mov x2, #0
