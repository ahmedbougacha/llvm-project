...
static llvm::Constant *buildBlockDescriptor(CodeGenModule &CGM,
  ...
  llvm::Constant *encodingString;
  if (CGM.getCodeGenOpts().DisableBlockSignatureString) {
    encodingString = llvm::ConstantPointerNull::get(i8p);
  } else {
    std::string typeAtEncoding =
      CGM.getContext().getObjCEncodingForBlock(blockInfo.getBlockExpr());
    encodingString = CGM.GetAddrOfConstantCString(typeAtEncoding).getPointer();
  }
...
llvm::Value *CodeGenFunction::EmitBlockLiteral(const CGBlockInfo &blockInfo) {
  ...
  for (const auto &CI : blockDecl->captures()) {
    ...
    // If it's a reference variable, copy the reference into the block field.
      ...
      llvm::Value *value =
          getAsNaturalPointerTo(src, refType->getPointeeType());
