...
// CHECK: @globalStripIntptr = global i64 ptrtoint (ptr ptrauth (ptr @globalStripPointer, i32 1, i64 105) to i64), align 8
...
void testStrip2(StripStruct *a) {
  ...
  // CHECK: [[PTR:%.*]] = getelementptr inbounds %struct.StripStruct, ptr [[A]], i32 0, i32 0
...
void testStrip3(StripStruct *a) {
  a->i += 1;
  // CHECK: [[A:%.*]] = load ptr, ptr %a.addr
  // CHECK: [[I:%.*]] = getelementptr inbounds %struct.StripStruct, ptr [[A]], i32 0, i32 1
...
void testSignAndStrip2(SignAndStripStruct *a) {
  a->ptr += 1;
  // CHECK: [[A:%.*]] = load ptr, ptr %a.addr
  // CHECK: %ptr = getelementptr inbounds %struct.SignAndStripStruct, ptr [[A]], i32 0, i32 0
  // CHECK: [[APTR:%.*]] = load ptr, ptr %ptr
  // CHECK: [[APTRI:%.*]] = ptrtoint ptr [[APTR]] to i64
...
void testSignAndStrip3(SignAndStripStruct *a) {
  a->i += 1;
  // CHECK: [[A:%.*]] = load ptr, ptr %a.addr
  // CHECK: [[I:%.*]] = getelementptr inbounds %struct.SignAndStripStruct, ptr [[A]], i32 0, i32 1
  // CHECK: [[I64:%.*]] = load i64, ptr [[I]]
...
void testSignAndAuth2(SignAndAuthStruct *a) {
  ...
  // CHECK: %i = getelementptr inbounds %struct.SignAndAuthStruct, ptr %0, i32 0, i32 1
...
void testSignAndAuth3(SignAndAuthStruct *a) {
  a->ptr += 1;
  // CHECK: %0 = load ptr, ptr %a.addr
  // CHECK: %ptr = getelementptr inbounds %struct.SignAndAuthStruct, ptr %0, i32 0, i32 0
...
void testCoercions1(StripStruct *a, SignAndStripStruct *b) {
  a->ptr = b->ptr;
  // CHECK: %0 = load ptr, ptr %a.addr
  // CHECK: %ptr = getelementptr inbounds %struct.StripStruct, ptr %0, i32 0, i32 0
  // CHECK: %1 = load ptr, ptr %b.addr
  // CHECK: %ptr1 = getelementptr inbounds %struct.SignAndStripStruct, ptr %1, i32 0, i32 0
...
void testCoercions2(StripStruct *a, SignAndAuthStruct *b) {
  ...
  // CHECK: %ptr1 = getelementptr inbounds %struct.StripStruct, ptr %1, i32 0, i32 0
  // CHECK: %2 = load ptr, ptr %ptr1
  // CHECK: %3 = ptrtoint ptr %ptr1 to i64
...
void testCoercions3(SignAndStripStruct *a, SignAndAuthStruct *b) {
  a->ptr = b->ptr;
  // CHECK: %0 = load ptr, ptr %a.addr
  // CHECK: %ptr = getelementptr inbounds %struct.SignAndStripStruct, ptr %0, i32 0, i32 0
  // CHECK: %1 = load ptr, ptr %b.addr
  // CHECK: %ptr1 = getelementptr inbounds %struct.SignAndAuthStruct, ptr %1, i32 0, i32 0
  // CHECK: %2 = load ptr, ptr %ptr1
  // CHECK: %3 = ptrtoint ptr %ptr1 to i64
...
void testCoercions4(StripStruct *a, SignAndStripStruct *b) {
  ...
  // CHECK: store ptr %b, ptr %b.addr
  // CHECK: %0 = load ptr, ptr %a.addr
  // CHECK: %i = getelementptr inbounds %struct.StripStruct, ptr %0, i32 0, i32 1
  // CHECK: %1 = load ptr, ptr %b.addr
  // CHECK: %i1 = getelementptr inbounds %struct.SignAndStripStruct, ptr %1, i32 0, i32 1
...
void testCoercions5(StripStruct *a, SignAndAuthStruct *b) {
  ...
  // CHECK: %i = getelementptr inbounds %struct.SignAndAuthStruct, ptr %0, i32 0, i32 1
  // CHECK: %1 = load ptr, ptr %a.addr
  // CHECK: %i1 = getelementptr inbounds %struct.StripStruct, ptr %1, i32 0, i32 1
  // CHECK: %2 = load i64, ptr %i1
  // CHECK: %4 = call i64 @llvm.ptrauth.strip(i64 %2, i32 1)
...
void testCoercions6(SignAndStripStruct *a, SignAndAuthStruct *b) {
  a->i = b->i;
  // CHECK: %0 = load ptr, ptr %a.addr
  // CHECK: %i = getelementptr inbounds %struct.SignAndStripStruct, ptr %0, i32 0, i32 1
  // CHECK: %1 = load ptr, ptr %b.addr
  // CHECK: %i1 = getelementptr inbounds %struct.SignAndAuthStruct, ptr %1, i32 0, i32 1
  // CHECK: %2 = load i64, ptr %i1
